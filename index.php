...
$pages = ceil((mysqli_fetch_assoc($resultMany)['count'])/4);
...
<body>
    ...
    <main class="container mb-5">
        <div class="products-grid">
            ...
            $query = "SELECT products.productId, products.userId, products.cateId, products.productName, 
                      ...
                      FROM `products` 
                      LEFT JOIN favorite ON products.productId = favorite.productId LIMIT 4 OFFSET $offset";
    ...
    <div class="pagination">
        <a href="?page=<?= $page+1?>" class="<?= $page == $pages ? 'disabled' : ''?>"> ‹ التالي  </a>
        <?php
            for ($i = $pages; $i >= 1; $i--){
        ?>
       ...
       <?php }?>
        ...
        <a href="?page=<?= $page-1?>" class="<?= $page == 1 ? 'disabled' : ''?>"> السابق › </a>
    </div>
